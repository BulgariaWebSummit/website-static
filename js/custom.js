...
$(document).ready(function () {
	...
	// Google Map //
	$('#map_canvas').gmap({
		'center': new google.maps.LatLng(42.649633,23.395533), // Change this to your desired latitude and longitude
		'zoom': 15,
	...
	$('#map_canvas').gmap().bind('init', function () {
		$('#map_canvas').gmap('addMarker', {
			'id': 'marker-1',
			'position': '42.649633,23.395533',
			'bounds': false,
			'icon': image
		}).click(function () {
			$('#map_canvas').gmap('openInfoWindow', {
				'content': '<h4>Bulgaria Web Summit</h4><p><strong>Интер Експо Център</strong><br>булевард „Цариградско шосе“ 147</p>'
			}, this);
		});
	});
	...
	if (window.matchMedia("(min-width: 480px)").matches) {
  	...
  	var mh = 0;
  	$('.speakers-list li').each(function(){
  		var h = $(this).height();
  		if(h > mh){
  			mh = h;
  		}
